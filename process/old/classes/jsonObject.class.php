...
class jsonObject {
	...
	function __construct($filename, $silent = false, $debug = false) {
		...
		$this->setFileIsJSON( $this->fileIsJSON($filename) );

		if ( $this->getFileIsJSON() ) {
	...
	private function fileIsJSON($filename) {
		$isJSON = false;
		//parse the path, and filename
		$this->parseFileNameAndPath($filename);
		if ( in_array( ($this->getFileExtension()), ($this->getAllowedFiles()) ) ) {
			$isJSON = true;
		} else {
			$isJSON = false;
		}
	...
	private function setFileIsJSON($bool)			{ $this->fileIsJSON = $bool;	}
	...
	private function getAllowedFiles()			{ return $this->allowedFiles;		} //no setter since this shouldnt be changed
	
	//public getter
	public function getjsonOBJ()				{ return $this->jsonOBJ;			}
	public function getFileIsJSON()				{ return $this->fileIsJSON;			}
}
?>